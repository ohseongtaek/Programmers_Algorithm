...
int solution(vector<string> board) 
{
    ...
    //string test = (char*)board[0][0];

    string leftuprightdown = to_string(board[0][0]) + to_string(board[1][1]) + to_string(board[2][2]);
    string rightupleftdown = to_string(board[0][2]) + to_string(board[1][1]) + to_string(board[2][0]);

    // 대각선 세기 
    if (leftuprightdown == "797979")
    {
        LineCntO++;
    }
    else if (leftuprightdown == "888888")
    {
        LineCntX++;
    }

    if (rightupleftdown == "797979")
    {
        LineCntO++;
    }
    else if (rightupleftdown == "888888")
    {
        LineCntX++;
    }
    ...
    for (int i = 0; i < board.size(); i++)
    {
        ...
        // 세로세기
        string temp = to_string(board[0][i]) + to_string(board[1][i]) + to_string(board[2][i]);
    ...
    // 둘다 승리하는 경우 말이 안됨 
    else if ((LineCntO > 0) && (LineCntX > 0))
    {
        answer = 0;
    }
    // O 가 승리했는데 X랑 갯수가 같은 경우 (더넣은경우지)
    else if ((LineCntO > 0) && (CntO <= CntX))
    {
        answer = 0;
    }
    // X가 승리했는데 OX가 같지 않은 경우 
    else if ((LineCntX > 0) && (CntO != CntX))
    {
        answer = 0;
    }
