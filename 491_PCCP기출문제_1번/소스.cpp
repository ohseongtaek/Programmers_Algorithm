...
int solution(vector<int> bandage, int health, vector<vector<int>> attacks) 
{
    ...
    {
        //공격일땐 아무것도 안하고 데미지만 입음
        if (attacks[attacksCnt][0] == i)
        {
            tHp -= attacks[attacksCnt][1];
            attacksCnt++;
            continueSuccess = 0;
        }
        else
...
int main()
{
    vector<vector<int>> v = { {5, 1, 5},
                            {3, 2, 7},
                            {4, 2, 7},
                            {1, 1, 1} };
    vector<int> h = { 30,20,20,5 };
    vector<vector<vector<int>>> att = { { {2, 10}, { 9, 15 }, { 10, 5 }, { 11, 5 }},
                                        { {1, 15}, { 5, 16 }, { 8, 6 } },
                                        { {1, 15}, { 5, 16 }, { 8, 6 }},
                                        { {1, 2}, { 3, 2 }} };
    
    vector<int> d = { 5,-1,-1,3 };

    for (int i = 0; i < h.size(); i++)
    {
        int dab = solution(v[i], h[i], att[i]);
        if (dab == d[i])
        {
            cout << " good " << endl;
        }
...
모든 공격이 끝난 직후 
남은 체력을 return 하도록 solution 함수를 완성해 주세요. 
만약 몬스터의 공격을 받고 캐릭터의 체력이 0 이하가 되어 죽는다면 
-1을 return 해주세요.
