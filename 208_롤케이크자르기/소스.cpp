...
int solution(vector<int> topping) 
{
	...
	for (int i = 0; i < topping.size(); i++)
	{
		...
		// 한개를 왼쪽친구한테 줬으니 준거를 오른쪽 친구한테서 뺏음 
		right[topping[i]]--;
...
int main()
{
	vector<vector<int>> b = { {1, 2, 1, 3, 1, 4, 1, 2}, {1, 2, 3, 1, 4} };
	vector<int> result = { 2, 0 };

	for (int i = 0; i < b.size(); i++)
	{
		int dab = solution(b[i]);
		if (dab == result[i])
		{
			cout << "good" << endl;
		}
...
만약 롤케이크의 네 번째 토핑(3)과 다섯 번째 토핑(1) 사이를 자르면 
[1, 2, 1, 3], [1, 4, 1, 2]로 나뉘게 됩니다. 
