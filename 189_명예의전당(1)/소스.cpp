...
vector<int> solution(int k, vector<int> score) 
{
	...
	{
		in.push_back(score[i]);
		sort(in.begin(), in.end(), cmp);
		
		if (in.size() < k)
		{
			answer.push_back(in[in.size()-1]);
		}
		else
		{
			answer.push_back(in[k-1]);
		}
	}
...
int main()
{
	...
	vector<vector<int>> sc = { { 10, 100, 20, 150, 1, 100, 200 }, 
	{ 0, 300, 40, 300, 20, 70, 150, 50, 500, 1000 } };

	vector<vector<int>> dab = { {10, 10, 10, 20, 20, 100, 100}, 
	{0, 0, 0, 0, 20, 40, 70, 70, 150, 300} };

	for (int i = 0; i < sc.size(); i++)
	{
		vector<int> d = solution(k[i], sc[i]);
